(() => {
  ...
  const updateHeaderMetrics = () => {
    ...
    document.documentElement.style.setProperty('--veloura-header-height', `${Math.ceil(header.getBoundingClientRect().height)}px`);
  };
  if (header) {
    updateHeaderMetrics();
  ...
  if (menuToggle && menu) {
    const setMenuState = open => {
      const shouldOpen = Boolean(open) && window.innerWidth < MOBILE_NAV_BREAKPOINT;
      menu.classList.toggle('open', shouldOpen);
      document.body.classList.toggle('menu-open', shouldOpen);
      if (menuBackdrop) menuBackdrop.classList.toggle('open', shouldOpen);
      menuToggle.setAttribute('aria-expanded', String(shouldOpen));
      menu.setAttribute('aria-hidden', String(!shouldOpen && window.innerWidth < MOBILE_NAV_BREAKPOINT));
    };
    ...
    document.addEventListener('click', event => {
      if (window.innerWidth < MOBILE_NAV_BREAKPOINT && menu.classList.contains('open') && !menu.contains(event.target) && !menuToggle.contains(event.target)) closeMenu();
    });
    document.addEventListener('keydown', event => { if (event.key === 'Escape') closeMenu(); });
    window.addEventListener('resize', () => {
      if (window.innerWidth >= MOBILE_NAV_BREAKPOINT) closeMenu();
      else menu.setAttribute('aria-hidden', String(!menu.classList.contains('open')));
      updateHeaderMetrics();
    }, { passive:true });
    setMenuState(false);
  }
